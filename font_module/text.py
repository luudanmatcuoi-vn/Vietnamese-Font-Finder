...
vietnamese_alphabet = "áàảãạăắằẳẵặâấầẩẫậđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴūáàảãạăắằẳẵặâấầẩẫậđéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴū"
...
class VietnameseRandomCorpusGeneratorWithEnglish(RandomCorpusGeneratorWithEnglish):
    def __init__(self, prob: float = 0.3, when_length_greater_than: int = 10):
        super().__init__(vietnamese_alphabet, prob, when_length_greater_than)
...
class CorpusGeneratorManager:
    def __init__(self):
        self.generators = {
            ...
            "vi": VietnameseRandomCorpusGeneratorWithEnglish(),
        }
